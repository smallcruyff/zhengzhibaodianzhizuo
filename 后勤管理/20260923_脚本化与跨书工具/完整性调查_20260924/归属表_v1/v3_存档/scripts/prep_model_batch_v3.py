#!/usr/bin/env python3
"""prep_model_batch_v3.py —— 缺口4"准备"部分 v3：只算队列大小、估算调用次数与token；不调模型。

相对 v2 的变更（对应 确认意见_v2.json major⑤）：
  0. --inputs-dir 默认读交付目录本地 inputs_snapshot/units.jsonl，不再硬编码另一次会话的
     /private/tmp 分类器目录；可用该参数改读其它位置。
  1. 身份错误/身份未确认行（admission以"身份"开头）从队列里排除，不占用模型预算估算。
相对 v1 的变更（对应 审查意见.json major④⑤、minor项，v2已实现，v3沿用）：
  2. 队列新增"COLLECTED复判"层：全部 归属_<某模块>==COLLECTED 的单元都排进复判队列，不再局限于
     conflict_script_out非空的80对。"已收只标待复核"要落到真的会被模型/人工复判一遍，不能停在标签上。
  3. missing_from_units_jsonl 排序后再输出（消除集合迭代顺序不确定的问题）。
  4. token估算明确加一条 caching_and_multiturn_caveat：本估算只算一次性内容token，不含缓存重读和
     多轮调用可能把实际用量放大一个数量级的提示（回应审查意见minor项）。
  5. 切片长度改为直接引用 units.jsonl 的 stem_len/rubric_len 全长（未变，v1已如此），并新增
...
def main():
    ...
    ap.add_argument('--inputs-dir', default='inputs_snapshot',
                     help='units.jsonl 所在目录，默认读交付目录本地快照；可指向其它位置重新估算')
    ...
    result = {
        ...
        'identity_excluded_rows': n_identity_excluded,
        'method': (
