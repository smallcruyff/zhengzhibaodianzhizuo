...
RULE_VERSION = '2026-09-24-attribution-v4'

CELL_RE = re.compile(r'^\s*([A-Za-z]+)(?:\[([^\]]*)\])?\s*$')


def cell_collects_subq(cell, subq):
    """确认意见_v3.json issue4修复（对应终审意见.json blocker“小问级已收由题级matrix整题展开而来”）：
    matrix.csv 模块列可能是 S[1/2]、M[1] 这样的小问级标注。有方括号时只把方括号里列出的小问视为
    已收；没有方括号时（如纯"S"/"M"）视为整题（含全部小问）已收，与旧行为一致。"""
    if not cell:
        return False
    m = CELL_RE.match(cell)
    if not m:
        return bool(cell)  # 未知格式兜底：保留原有"非空即收"行为，不静默丢数据
    _tag, brack = m.groups()
    if brack is None:
        return True
    subs = {x.strip() for x in brack.split('/') if x.strip()}
    if not subq:
        # 本行是"整题"粒度（没有独立小问行，比如题库对该题未按小问切分），但 matrix 只标了部分
        # 小问——不能算整题已收；classified_prod.jsonl 对多小问主观题一律按小问切分出行，这种情形
        # 理论上不会发生，这里保守返回 False 而不是 True，避免重新引入同一类"整题展开"错误。
        return False
    return str(subq) in subs
...
def main():
    ...
    for u in classified:
        ...
        for m in MODS:
            ...
            collected_direct = bool(book) and cell_collects_subq(mrow.get(book), subq)
            collected_partner = bool(book) and cell_collects_subq(partner_row.get(book), subq)
